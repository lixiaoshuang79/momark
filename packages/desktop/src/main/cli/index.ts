...
const cli = (): ParsedArgs => {
  ...
  if (process.env.NODE_ENV === 'development') {
    ...
    argv = ['--user-data-dir', path.join(getPath('appData'), 'momark-dev')]
  }

  const args = parseArgs(argv, true)
  if (args['--help']) {
    write(`Usage: momark [commands] [path ...]
...
`)
  ...
  if (args['--version']) {
    writeLine(`MoMark: ${MARKTEXT_VERSION_STRING}`)
  ...
  if (!args['--user-data-dir']) {
    const portablePath = path.join(app.getAppPath(), '..', '..', 'momark-user-data')
