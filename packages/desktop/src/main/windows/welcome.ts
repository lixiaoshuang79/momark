...
import { welcomeWinOptions, isLinux } from '../config'
import log from 'electron-log'

/**
 * MoMark welcome window (PHASE2-SPEC §9): 660×440pt, three entry buttons on the
 * left (new document / open folder / open file), recent documents on the right,
 * first-launch empty state when no documents were opened before.
 */
class WelcomeWindow extends BaseWindow {
  ...
  constructor(accessor: Accessor) {
    super(accessor)
    this.type = WindowType.WELCOME
  }

  createWindow(): BrowserWindow {
    const { env, preferences } = this._accessor
    const winOptions: BrowserWindowConstructorOptions = Object.assign({}, welcomeWinOptions)
